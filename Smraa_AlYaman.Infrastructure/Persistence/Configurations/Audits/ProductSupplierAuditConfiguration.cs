using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;
using Smraa_AlYaman.Domain.ProductSuppliers.Audits;
using System;
using System.Collections.Generic;
using System.Text;

namespace Smraa_AlYaman.Infrastructure.Persistence.Configurations.Audits
{
    public class ProductSupplierAuditConfiguration : IEntityTypeConfiguration<ProductSupplayerAudit>
    {
        public void Configure(EntityTypeBuilder<ProductSupplayerAudit> builder)
        {

            builder.OwnsOne(p => p.EntityId)
                   .Property(p => p.ProductId)
                   .HasColumnName("ProductId")
                   .IsRequired();

            builder.OwnsOne(p => p.EntityId)
                   .Property(p => p.SupplayerId)
                   .HasColumnName("SupplayerId")
                   .IsRequired();

            builder.Property(p => p.AuditId)
                    .ValueGeneratedNever();
            builder.HasKey(p => p.AuditId);


            builder.Property(p => p.From)
                    .IsRequired();
            builder.Property(p => p.ChangedAt)
                    .IsRequired();


            builder.Property(p => p.IsRecovered)
                    .IsRequired()
                    .HasDefaultValue(false);
        }
    }
}
